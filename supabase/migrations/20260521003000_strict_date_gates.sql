-- ============================================================================
-- Gates de transition stricts : toute date antérieure doit être renseignée
-- avant chaque avancement de phase. Ainsi un projet qui a "sauté" une date
-- ne pourra pas avancer sans la backfiller.
-- ============================================================================
...
BEGIN
  ...
  -- design exige : compromis + bien validé
  IF p_new_phase IN ('design','travaux','livraison','mise_en_location','termine') THEN
    ...
    IF v_project.compromis_date IS NULL THEN
      RAISE EXCEPTION 'Date de signature du compromis manquante (requise pour Design et au-delà). Renseignez-la dans la carte "Dates clés".';
    END IF;
  END IF;

  -- travaux exige en plus : acte authentique
  IF p_new_phase IN ('travaux','livraison','mise_en_location','termine') THEN
    IF v_project.acte_authentique_date IS NULL THEN
      RAISE EXCEPTION 'Date de signature de l''acte authentique manquante (requise pour Travaux et au-delà). Renseignez-la dans la carte "Dates clés".';
    END IF;
  END IF;

  -- livraison exige en plus : lancement et fin de chantier
  IF p_new_phase IN ('livraison','mise_en_location','termine') THEN
    IF v_project.travaux_start_date IS NULL THEN
      RAISE EXCEPTION 'Date de lancement de chantier manquante (requise pour Livraison et au-delà).';
    END IF;
    IF v_project.travaux_end_date IS NULL THEN
      RAISE EXCEPTION 'Date de livraison du chantier (fin travaux) manquante (requise pour Livraison et au-delà).';
    END IF;
  END IF;

  -- mise_en_location exige en plus : remise des clés client
  IF p_new_phase IN ('mise_en_location','termine') THEN
    IF v_project.livraison_date IS NULL THEN
      RAISE EXCEPTION 'Date de remise des clés au client manquante (requise pour Mise en location).';
    END IF;
  END IF;
  ...
  -- Création de l'enquête de satisfaction sur la phase qui se termine
  v_survey_phase := NULL;
