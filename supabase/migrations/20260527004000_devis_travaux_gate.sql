-- ============================================================================
-- Phase Design — gate "devis travaux" obligatoire avant passage en Travaux
-- + correction du bug 'approved' vs 'validated' sur le gate plan_bet
-- ============================================================================

-- 1. Ajouter `devis_travaux` dans la liste des types de documents autorisés
...
-- 2. Tâche : faire valider le devis travaux par le client (phase design, bloquante)
INSERT INTO task_templates (phase, title, description, default_assigned_role, order_index, is_blocking)
VALUES
  ('design', 'Faire valider le devis travaux par le client',
   'Uploader le devis global travaux (tous lots consolidés) dans Documents (type "devis_travaux") et le marquer "à valider par le client". La validation client est obligatoire pour passer en phase Travaux.',
   'chef_projet', 35, true)
ON CONFLICT DO NOTHING;

-- 3. Réécriture de advance_project_phase
--    • Correction : le check sur plan_bet utilisait 'approved' alors que l'app
--      écrit 'validated' — le gate était de facto inopérant.
--    • Ajout : devis_travaux doit être présent ET validé par le client pour
--      passer de Design à Travaux.
...
BEGIN
  ...
  IF v_project.current_phase = 'design' AND p_new_phase = 'travaux' THEN
    ...
    -- Plan BET : présent + validé client
