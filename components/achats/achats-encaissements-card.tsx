...
import { Trash2, Pencil, CalendarClock, CalendarPlus, BanknoteIcon } from 'lucide-react';
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Input, Label } from '@/components/ui/input';
import { formatMad, formatDate } from '@/lib/utils/format';
import {
  addAchatEncaissementAction,
  updateAchatEncaissementAction,
  deleteAchatEncaissementAction,
...
export function AchatsEncaissementsCard({
  projectId,
  encaissements,
}: {
  projectId: string;
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      const r = await addAchatEncaissementAction(data);
  ...
  function remove(id: string) {
    ...
    start(async () => { await deleteAchatEncaissementAction(id); });
  }
    ...
    <Card>
      <CardHeader>
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <CardTitle>Encaissements client (achats)</CardTitle>
              ...
              {planifies.map((e) => (
                <li key={e.id} className="px-3 py-2 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <FinanceAuditButton table="achats_encaissements" recordId={e.id} />
                    <AchatEncaissementEditButton encaissement={e} projectId={projectId} />
                    <button
                      onClick={() => remove(e.id)}
                      disabled={pending}
      ...
      {openMode && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setOpenMode(null)}>
...
function AchatEncaissementEditButton({
  encaissement,
  projectId,
}: {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const r = await updateAchatEncaissementAction(payload);
