...
BEGIN
  ...
  FOR i IN 1..10 LOOP
    ...
    INSERT INTO travaux_lots (project_id, numero, category, description, artisan_name, artisan_type,
      ...
      (v_project, 1, 'demolition_cloisons',  '[DEMO] Demolition cloisons + curage',
        'Hassan Demolition SARL', 'artisan_local', 'DV-' || i || '-001',
        25000, 22000, 32000, 'termine',
        (CURRENT_DATE - INTERVAL '60 days')::date, (CURRENT_DATE - INTERVAL '50 days')::date,
        (CURRENT_DATE - INTERVAL '48 days')::date, '[DEMO] Lot termine sans surprise'),
      (v_project, 2, 'gros_oeuvre_maconnerie', '[DEMO] Reprise cloisons + chapes',
        'Atlas Maconnerie', 'entreprise_generale', 'DV-' || i || '-002',
        80000, 75000, 105000, 'en_cours',
        (CURRENT_DATE - INTERVAL '45 days')::date, (CURRENT_DATE + INTERVAL '5 days')::date,
        NULL, '[DEMO] Avancement 60 pct'),
      (v_project, 3, 'electricite', '[DEMO] Refonte installation electrique complete',
    ...
    INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
      ...
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 1),
        'Hassan Demolition SARL', 'artisan_local', 'autre',
        'Acompte 1/3 demolition', 'MAD', 8800, 8800, 10, now() - INTERVAL '58 days',
        'acompte', 'paid', 1, 40,
        (CURRENT_DATE - INTERVAL '58 days')::date, (CURRENT_DATE - INTERVAL '58 days')::date,
        '[DEMO] Acompte demarrage'),
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 1),
        'Hassan Demolition SARL', 'artisan_local', 'autre',
        'Acompte 2/3 demolition', 'MAD', 8800, 8800, 10, now() - INTERVAL '52 days',
        'acompte', 'paid', 2, 40,
        (CURRENT_DATE - INTERVAL '52 days')::date, (CURRENT_DATE - INTERVAL '52 days')::date,
        '[DEMO] Mi-parcours'),
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 1),
        'Hassan Demolition SARL', 'artisan_local', 'autre',
        'Solde demolition', 'MAD', 4400, 4400, 10, now() - INTERVAL '48 days',
        'solde', 'paid', 3, 20,
    ...
    INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
      ...
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 2),
        'Atlas Maconnerie', 'entreprise_generale', 'gros_oeuvre',
        'Acompte 1 maconnerie', 'MAD', 30000, 30000, 10, now() - INTERVAL '44 days',
        'acompte', 'paid', 1, 40,
        (CURRENT_DATE - INTERVAL '44 days')::date, (CURRENT_DATE - INTERVAL '44 days')::date,
        '[DEMO]'),
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 2),
        'Atlas Maconnerie', 'entreprise_generale', 'gros_oeuvre',
        'Acompte 2 maconnerie', 'MAD', 30000, 30000, 10, now() - INTERVAL '25 days',
        'acompte', 'paid', 2, 40,
        (CURRENT_DATE - INTERVAL '25 days')::date, (CURRENT_DATE - INTERVAL '25 days')::date,
        '[DEMO]'),
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 2),
        'Atlas Maconnerie', 'entreprise_generale', 'gros_oeuvre',
        'Solde maconnerie', 'MAD', 15000, 0, 10, NULL,
        'solde', 'pending', 3, 20,
    ...
    INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
      ...
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 3),
        ...
        'Acompte 1 electricite', 'MAD', 24000, 24000, 10, now() - INTERVAL '38 days',
        'acompte', 'paid', 1, 40,
  ...
  RAISE NOTICE 'Demo seed termine : 10 clients, 10 biens, 10 projets en travaux, 50 lots';
END $$;
...
DECLARE
  ...
  -- Variables locales par projet (recalculées dans la boucle)
  base_mad         NUMERIC;  -- budget travaux global en MAD
  ...
  lot2_pct_paid    NUMERIC;  -- % du devis lot 2 déjà versé
  lot3_pct_paid    NUMERIC;  -- % du devis lot 3 déjà versé
  encaisse_1       NUMERIC;  encaisse_2 NUMERIC;
  days_offset      INTEGER;  -- décalage de dates par projet
  lot2_status      TEXT;
  lot3_status      TEXT;
  lot4_status      TEXT;
...
BEGIN
  ...
  FOR i IN 1..10 LOOP
    INSERT INTO properties (
      ...
      'Bien d''exception au coeur de Marrakech. ' || prop_superficies[i] || ' m2 habitables, '
        || prop_suites[i] || ' suites, terrasse de ' || prop_terrasses[i] || ' m2. '
        || 'Quartier ' || prop_quartiers[i] || ', ideal pour location courte duree.',
      2.5,
      (CURRENT_DATE - INTERVAL '200 days')::date,
      (CURRENT_DATE - INTERVAL '180 days')::date,
      (CURRENT_DATE - INTERVAL '160 days')::date,
  ...
  -- ─── 3. PROJETS + lots + paiements (avec variance par projet) ────────
  FOR i IN 1..10 LOOP
    base_mad := prop_travaux[i] * 10;  -- 280k a 680k MAD selon le bien

    -- Répartition des lots (% du budget global)
    lot1_devis   := ROUND(base_mad * 0.08, 0);   lot1_facture := ROUND(base_mad * 0.12, 0);
    lot2_devis   := ROUND(base_mad * 0.32, 0);   lot2_facture := ROUND(base_mad * 0.42, 0);
    lot3_devis   := ROUND(base_mad * 0.22, 0);   lot3_facture := ROUND(base_mad * 0.30, 0);
    lot4_devis   := ROUND(base_mad * 0.18, 0);   lot4_facture := ROUND(base_mad * 0.25, 0);
    lot5_devis   := ROUND(base_mad * 0.20, 0);   lot5_facture := ROUND(base_mad * 0.27, 0);

    -- Avancement : % du lot 2/3 déjà payé varie par projet
    lot2_pct_paid := CASE i
      WHEN 1  THEN 100  WHEN 2  THEN 80   WHEN 3  THEN 100  WHEN 4  THEN 60
      WHEN 5  THEN 40   WHEN 6  THEN 80   WHEN 7  THEN 40   WHEN 8  THEN 60
      WHEN 9  THEN 20   WHEN 10 THEN 0    ELSE 50 END;
    lot3_pct_paid := CASE i
      WHEN 1  THEN 80   WHEN 2  THEN 60   WHEN 3  THEN 80   WHEN 4  THEN 40
      WHEN 5  THEN 40   WHEN 6  THEN 0    WHEN 7  THEN 0    WHEN 8  THEN 0
      WHEN 9  THEN 0    WHEN 10 THEN 0    ELSE 0 END;

    -- Statuts lots variés
    lot2_status := CASE WHEN i <= 3 THEN 'termine'
                       WHEN i <= 7 THEN 'en_cours'
                       ELSE 'en_attente' END;
    lot3_status := CASE WHEN i <= 5 THEN 'en_cours'
                       WHEN i <= 7 THEN 'en_attente'
                       ELSE 'a_planifier' END;
    lot4_status := CASE WHEN i <= 4 THEN 'devis_recu'
                       ELSE 'a_planifier' END;

    -- Encaissements client : ~75% de ce qui a été facturé pour les lots 1+2+3 (avec variation)
    encaisse_1 := ROUND((lot1_facture + lot2_facture * lot2_pct_paid / 100.0) * 0.6, 0);
    encaisse_2 := ROUND((lot3_facture * lot3_pct_paid / 100.0 + lot2_facture * 0.2) * 0.7, 0);

    days_offset := (10 - i) * 4;  -- projet 1 avance de 36j, projet 10 jamais

    -- INSERT projet (trigger init_new_project cree onboarding + acompte 5000€)
    INSERT INTO projects (
      client_id, property_id, assigned_chef_projet,
      current_phase, status,
      ...
      client_ids[i], property_ids[i], v_chef,
      'onboarding', 'actif',
      prop_travaux[i], base_mad, 28 + (i % 5) * 2,
      prop_addresses[i],
      8800, 12200
    ) RETURNING id INTO v_project;
    project_ids := array_append(project_ids, v_project);
    ...
    UPDATE projects SET
      current_phase = 'travaux',
      onboarding_date       = (CURRENT_DATE - INTERVAL '1 day' * (180 + days_offset))::date,
      compromis_date        = (CURRENT_DATE - INTERVAL '1 day' * (130 + days_offset))::date,
      acte_authentique_date = (CURRENT_DATE - INTERVAL '1 day' * (90  + days_offset))::date,
      travaux_start_date    = (CURRENT_DATE - INTERVAL '1 day' * (60  + days_offset))::date,
      travaux_end_date      = (CURRENT_DATE + INTERVAL '1 day' * (45  - days_offset))::date
    WHERE id = v_project;

    UPDATE project_phases_history
      SET completed_at = (now() - INTERVAL '1 day' * (150 + days_offset))
      WHERE project_id = v_project AND phase = 'onboarding' AND completed_at IS NULL;

    INSERT INTO project_phases_history (project_id, phase, started_at, completed_at) VALUES
      (v_project, 'sourcing', now() - INTERVAL '1 day' * (150 + days_offset), now() - INTERVAL '1 day' * (100 + days_offset)),
      (v_project, 'design',   now() - INTERVAL '1 day' * (100 + days_offset), now() - INTERVAL '1 day' * (60  + days_offset)),
      (v_project, 'travaux',  now() - INTERVAL '1 day' * (60  + days_offset), NULL);

    -- Honoraires Stoniz : acompte + compromis + design + chantier (chantier variable)
    UPDATE payments
      SET amount_paid = 5000, paid_at = (CURRENT_DATE - INTERVAL '170 days')::date,
          notes = '[DEMO] Acompte initial'
      WHERE project_id = v_project AND type = 'acompte_stoniz';
    ...
    INSERT INTO payments (project_id, type, amount_expected, amount_paid, due_at_phase, due_date, paid_at, notes) VALUES
      ...
      (v_project, 'honoraires_3d', 3800, 3800, 'design',
        ...
        '[DEMO] Design paye'),
      (v_project, 'honoraires_chantier', 4200,
        CASE WHEN i <= 6 THEN 4200 WHEN i = 7 THEN 2100 ELSE 0 END, 'travaux',
        (CURRENT_DATE - INTERVAL '55 days')::date,
        CASE WHEN i <= 7 THEN (CURRENT_DATE - INTERVAL '50 days')::date ELSE NULL END,
        '[DEMO] Chantier');

    -- ─── 4. LOTS TRAVAUX (5 lots par projet, montants varies) ─────────
    INSERT INTO travaux_lots (project_id, numero, category, description, artisan_name, artisan_type,
                              devis_number, budget_estimate_mad, devis_artisan_mad, facture_client_mad,
                              status, date_debut_estime, date_fin_estimee, date_fin_reelle, notes) VALUES
      (v_project, 1, 'demolition_cloisons',  '[DEMO] Demolition cloisons + curage',
        'Hassan Demolition SARL', 'artisan_local', 'DV-' || i || '-001',
        ROUND(base_mad * 0.10, 0), lot1_devis, lot1_facture, 'termine',
        (CURRENT_DATE - INTERVAL '60 days')::date, (CURRENT_DATE - INTERVAL '50 days')::date,
        (CURRENT_DATE - INTERVAL '48 days')::date, '[DEMO] Lot termine'),

      (v_project, 2, 'gros_oeuvre_maconnerie', '[DEMO] Reprise cloisons + chapes',
        'Atlas Maconnerie', 'entreprise_generale', 'DV-' || i || '-002',
        ROUND(base_mad * 0.35, 0), lot2_devis, lot2_facture, lot2_status,
        (CURRENT_DATE - INTERVAL '45 days')::date, (CURRENT_DATE + INTERVAL '5 days')::date,
        CASE WHEN lot2_status = 'termine' THEN (CURRENT_DATE - INTERVAL '5 days')::date END,
        '[DEMO] ' || lot2_pct_paid || '% paye'),

      (v_project, 3, 'electricite', '[DEMO] Refonte installation electrique',
        'ElecPro Marrakech', 'sous_traitant_ext', 'DV-' || i || '-003',
        ROUND(base_mad * 0.25, 0), lot3_devis, lot3_facture, lot3_status,
        (CURRENT_DATE - INTERVAL '40 days')::date, (CURRENT_DATE + INTERVAL '10 days')::date,
        NULL, '[DEMO]'),

      (v_project, 4, 'plomberie_sanitaire', '[DEMO] Sanitaires + alimentation',
        'Aqua Plomb', 'artisan_local', 'DV-' || i || '-004',
        ROUND(base_mad * 0.20, 0), lot4_devis, lot4_facture, lot4_status,
        (CURRENT_DATE - INTERVAL '20 days')::date, (CURRENT_DATE + INTERVAL '20 days')::date,
        NULL, '[DEMO]'),

      (v_project, 5, 'carrelage_revetements', '[DEMO] Carrelage + zellige SDB',
        'Zellige Freres', 'artisan_local', 'DV-' || i || '-005',
        ROUND(base_mad * 0.22, 0), lot5_devis, lot5_facture, 'a_planifier',
        (CURRENT_DATE + INTERVAL '5 days')::date, (CURRENT_DATE + INTERVAL '35 days')::date,
        NULL, '[DEMO]');

    -- ─── 5. ACOMPTES TRAVAUX (en MAD) ─────────────────────────────────
    -- Lot 1 : termine integralement (3 acomptes 40/40/20)
    INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
      description, currency, amount_total, amount_paid, exchange_rate_eur, exchange_rate_at,
      payment_type, status, acompte_number, acompte_pct, scheduled_date, paid_at, notes) VALUES
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 1),
        'Hassan Demolition SARL', 'artisan_local', 'autre',
        'Acompte 1/3 demolition', 'MAD', ROUND(lot1_devis * 0.40, 0), ROUND(lot1_devis * 0.40, 0), 10, now() - INTERVAL '58 days',
        'acompte', 'paid', 1, 40,
        (CURRENT_DATE - INTERVAL '58 days')::date, (CURRENT_DATE - INTERVAL '58 days')::date,
        '[DEMO] Acompte demarrage'),
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 1),
        'Hassan Demolition SARL', 'artisan_local', 'autre',
        'Acompte 2/3 demolition', 'MAD', ROUND(lot1_devis * 0.40, 0), ROUND(lot1_devis * 0.40, 0), 10, now() - INTERVAL '52 days',
        'acompte', 'paid', 2, 40,
        (CURRENT_DATE - INTERVAL '52 days')::date, (CURRENT_DATE - INTERVAL '52 days')::date,
        '[DEMO] Mi-parcours'),
      (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 1),
        'Hassan Demolition SARL', 'artisan_local', 'autre',
        'Solde demolition', 'MAD', ROUND(lot1_devis * 0.20, 0), ROUND(lot1_devis * 0.20, 0), 10, now() - INTERVAL '48 days',
        'solde', 'paid', 3, 20,
        (CURRENT_DATE - INTERVAL '48 days')::date, (CURRENT_DATE - INTERVAL '48 days')::date,
        '[DEMO] Solde final');

    -- Lot 2 : variable selon lot2_pct_paid (0 a 100%)
    IF lot2_pct_paid >= 40 THEN
      INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
        description, currency, amount_total, amount_paid, exchange_rate_eur, exchange_rate_at,
        payment_type, status, acompte_number, acompte_pct, scheduled_date, paid_at, notes) VALUES
        (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 2),
          'Atlas Maconnerie', 'entreprise_generale', 'gros_oeuvre',
          'Acompte 1 maconnerie', 'MAD', ROUND(lot2_devis * 0.40, 0), ROUND(lot2_devis * 0.40, 0), 10, now() - INTERVAL '44 days',
          'acompte', 'paid', 1, 40,
          (CURRENT_DATE - INTERVAL '44 days')::date, (CURRENT_DATE - INTERVAL '44 days')::date,
          '[DEMO]');
    END IF;
    IF lot2_pct_paid >= 80 THEN
      INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
        description, currency, amount_total, amount_paid, exchange_rate_eur, exchange_rate_at,
        payment_type, status, acompte_number, acompte_pct, scheduled_date, paid_at, notes) VALUES
        (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 2),
          'Atlas Maconnerie', 'entreprise_generale', 'gros_oeuvre',
          'Acompte 2 maconnerie', 'MAD', ROUND(lot2_devis * 0.40, 0), ROUND(lot2_devis * 0.40, 0), 10, now() - INTERVAL '25 days',
          'acompte', 'paid', 2, 40,
          (CURRENT_DATE - INTERVAL '25 days')::date, (CURRENT_DATE - INTERVAL '25 days')::date,
          '[DEMO]');
    END IF;
    IF lot2_pct_paid >= 20 THEN
      INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
        description, currency, amount_total, amount_paid, exchange_rate_eur, exchange_rate_at,
        payment_type, status, acompte_number, acompte_pct, scheduled_date, paid_at, notes) VALUES
        (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 2),
          'Atlas Maconnerie', 'entreprise_generale', 'gros_oeuvre',
          'Solde maconnerie', 'MAD', ROUND(lot2_devis * 0.20, 0),
          CASE WHEN lot2_pct_paid = 100 THEN ROUND(lot2_devis * 0.20, 0) ELSE 0 END,
          10, CASE WHEN lot2_pct_paid = 100 THEN now() - INTERVAL '5 days' END,
          'solde', CASE WHEN lot2_pct_paid = 100 THEN 'paid' ELSE 'pending' END, 3, 20,
          (CURRENT_DATE + INTERVAL '5 days')::date,
          CASE WHEN lot2_pct_paid = 100 THEN (CURRENT_DATE - INTERVAL '5 days')::date END,
          '[DEMO]');
    END IF;

    -- Lot 3 : variable selon lot3_pct_paid
    IF lot3_pct_paid >= 40 THEN
      INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
        description, currency, amount_total, amount_paid, exchange_rate_eur, exchange_rate_at,
        payment_type, status, acompte_number, acompte_pct, scheduled_date, paid_at, notes) VALUES
        (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 3),
          'ElecPro Marrakech', 'sous_traitant_ext', 'electricite',
          'Acompte 1 electricite', 'MAD', ROUND(lot3_devis * 0.40, 0), ROUND(lot3_devis * 0.40, 0), 10, now() - INTERVAL '38 days',
          'acompte', 'paid', 1, 40,
          (CURRENT_DATE - INTERVAL '38 days')::date, (CURRENT_DATE - INTERVAL '38 days')::date,
          '[DEMO] Demarrage installation');
    END IF;
    IF lot3_pct_paid >= 80 THEN
      INSERT INTO travaux_payments (project_id, lot_id, artisan_name, artisan_type, category,
        description, currency, amount_total, amount_paid, exchange_rate_eur, exchange_rate_at,
        payment_type, status, acompte_number, acompte_pct, scheduled_date, paid_at, notes) VALUES
        (v_project, (SELECT id FROM travaux_lots WHERE project_id = v_project AND numero = 3),
          'ElecPro Marrakech', 'sous_traitant_ext', 'electricite',
          'Acompte 2 electricite', 'MAD', ROUND(lot3_devis * 0.40, 0), ROUND(lot3_devis * 0.40, 0), 10, now() - INTERVAL '18 days',
          'acompte', 'paid', 2, 40,
          (CURRENT_DATE - INTERVAL '18 days')::date, (CURRENT_DATE - INTERVAL '18 days')::date,
          '[DEMO]');
    END IF;

    -- ─── 6. ENCAISSEMENTS CLIENT (montants varies par projet) ─────────
    IF encaisse_1 > 0 THEN
      INSERT INTO travaux_encaissements (project_id, amount_mad, received_at, payment_method, notes) VALUES
        (v_project, encaisse_1, (CURRENT_DATE - INTERVAL '55 days')::date, 'virement',
          '[DEMO] Versement 1 client travaux');
    END IF;
    IF encaisse_2 > 0 AND i <= 8 THEN
      INSERT INTO travaux_encaissements (project_id, amount_mad, received_at, payment_method, notes) VALUES
        (v_project, encaisse_2, (CURRENT_DATE - INTERVAL '30 days')::date, 'virement',
          '[DEMO] Versement 2 client travaux');
    END IF;
    IF i <= 4 THEN
      -- Les 4 projets les plus avances ont un 3e versement
      INSERT INTO travaux_encaissements (project_id, amount_mad, received_at, payment_method, notes) VALUES
        (v_project, ROUND(base_mad * 0.15, 0), (CURRENT_DATE - INTERVAL '10 days')::date, 'virement',
          '[DEMO] Versement 3 client travaux');
    END IF;
  ...
  RAISE NOTICE 'Demo seed termine : 10 clients, 10 biens, 10 projets en travaux, ~50 lots avec montants varies';
END $$;